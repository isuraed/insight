...
public class ReviewsByProductIdImporter {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 2) {
            System.out.println("Usage: com.isuraed.insight.ReviewsByProductIdImporter <hdfs_input_path> <hbase_table_name>");
            System.exit(1);
        }

        Logger logger = Logger.getLogger(ReviewsByProductIdImporter.class.getName());
            ...
            for (int i = 0; i < status.length; i++) {
                ...
                while (line != null) {
                    ...
                    for (int j = 0; j < jsonList.size(); j++) {
                        ...
                        // Add padding to timestamp so that the columns are ordered in HBase.
                        long timestamp = (Long)jsonObj.get("timestamp");
                        String paddedTimestamp = String.valueOf(timestamp);
                        if (paddedTimestamp.length() < 10) {
                            for (int n = 0; n < 10 - paddedTimestamp.length(); n++) {
                                paddedTimestamp = "0" + paddedTimestamp;
                            }
                        }

                        String userId = (String)jsonObj.get("userId");
                        byte[] colKey = Bytes.toBytes(paddedTimestamp + "_" + userId);
