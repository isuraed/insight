...
public class ProductsByTitleImporter {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 2) {
            System.out.println("Usage: com.isuraed.insight.ReviewsByProductTitleImporter <hdfs_input_path> <hbase_table_name>");
            System.exit(1);
        }
            ...
            for (int i = 0; i < status.length; i++) {
                ...
                while (line != null) {
                    ...
                    for (int j = 1; j < row.length; j++) {
                        byte[] colKey = Bytes.toBytes(row[j]);
                        byte[] colValue = Bytes.toBytes(row[j]);
