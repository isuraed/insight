...
public class ProductsByKeywordImporter {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 2) {
            System.out.println("Usage: com.isuraed.insight.ProductsByKeywordImporter <hdfs_input_path> <hbase_table_name>");
            System.exit(1);
        }

        Logger logger = Logger.getLogger(ProductsByKeywordImporter.class.getName());
        ...
        try {
            ...
            for (int i = 0; i < status.length; i++) {
                ...
                while (line != null) {
                    ...
                    // First element is row key (keyword). Remaining elements are productId's.
                    for (int j = 1; j < row.length; j++) {
                        String productId = row[j];
                        byte[] colKey = Bytes.toBytes(productId);
                        byte[] colValue = Bytes.toBytes(productId);
